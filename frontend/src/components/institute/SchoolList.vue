<template>
  <v-container class="containerSetup" fluid>
        ...
        <v-row>
          ...
          <v-col class="d-flex justify-end">
            <PrimaryButton
              ...
              icon="mdi-plus-thick"
              text="New School"
              @click.native="newSchoolSheet = !newSchoolSheet"
            ></PrimaryButton>
          </v-col>
        </v-row>
          ...
          <v-col class="pt-0">
              ...
              <v-col cols="12" md="4" class="d-flex justify-start">
                ...
                >
                  <template v-slot:item="data">
                    <v-icon :color="getStatusColorAuthorityOrSchool(data.item.status)"> mdi-circle-medium </v-icon>
                  ...
                  <template v-slot:item="{ item }">
                    <v-row>
                      <v-col cols="12" class="pr-0">
                        <v-icon :color="getStatusColorAuthorityOrSchool(item.name)"> mdi-circle-medium </v-icon>
            ...
            <v-row class="mt-n6">
              <v-col cols="12" md="4" class="d-flex justify-start">
                ...
                >
                  <template v-slot:item="data">
                    <v-icon :color="getDistrictStatusColor(data.item.status)"> mdi-circle-medium </v-icon>
              ...
              <v-col class="d-flex justify-end">
                ...
                <PrimaryButton
                  ...
                  text="Search"
                  @click.native="searchButtonClick"
    ...
    <!--    new school sheet -->
    <v-bottom-sheet v-model="newSchoolSheet" inset no-click-animation scrollable persistent width="50% !important">
      <NewSchoolPage
        v-if="newSchoolSheet"
...
export default {
  ...
  computed: {
    ...
    ...mapState('institute', [
      'facilityTypeCodes',
      'activeSchoolCategoryTypeCodes',
      'schoolCategoryTypeCodes',
      'schoolReportingRequirementTypeCodes',
    ]),
    ...
    schoolFacilityTypes() {
      ...
      let facilityTypes = this.schoolCategoryFacilityTypesMap[this.schoolCategoryTypeFilter]?.map(
        (schoolCatFacilityTypeCode) =>
          this.activeFacilityTypeCodes.find((facTypCode) => facTypCode.facilityTypeCode === schoolCatFacilityTypeCode)
      );
  ...
  methods: {
    ...
    setSchoolStatuses() {
      this.schoolStatus = [
        { name: 'Open', code: 'Open' },
        { name: 'Opening', code: 'Opening' },
        { name: 'Closing', code: 'Closing' },
        { name: 'Closed', code: 'Closed' },
        { name: 'Never Opened', code: 'NeverOpened' },
      ];
    },
    ...
    getActiveAuthorityDropDownItems() {
      ApiService.getActiveAuthorities()
        .then((response) => {
          for (const authority of response.data) {
    ...
    hasSearchValue() {
      return (
        this.schoolSearchParams.schoolID ||
    ...
    getSchoolList() {
      ...
      ApiService.apiAxios
        .get(Routes.institute.SCHOOL_PAGINATED_DATA_URL, {
          params: {
    ...
    getFacilityType(school) {
      return this.allSchoolFacilityTypes.find((facility) => facility.facilityTypeCode === school.facilityTypeCode)
        .label;
    },
    getSchoolCategory(school) {
      return this.schoolCategoryTypes.find((category) => category.schoolCategoryCode === school.schoolCategoryCode)
        .label;
    },
    formatPhoneNumber,
    sortByNameValue,
    ...
    searchEnabled() {
      return (
        (this.schoolCodeNameFilter !== '' && this.schoolCodeNameFilter !== null) ||
        (this.schoolStatusFilter !== '' && this.schoolStatusFilter !== null) ||
        (this.schoolFacilityTypeFilter !== '' && this.schoolFacilityTypeFilter !== null) ||
        (this.districtCodeNameFilter !== '' && this.districtCodeNameFilter !== null) ||
