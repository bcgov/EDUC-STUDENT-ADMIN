      ...
      <v-card>
        ...
        <v-card-text class="pt-2" :style="[editState ? {'background-color': '#e7ebf0'} : {'background-color': 'white'}]" >
          ...
          <v-list-item-group
              ...
              multiple
          >
            <v-list-item :disabled="roleDisabled(newrole)" v-for="newrole in districtRoles" :key="newrole.edxRoleCode" :value="newrole.edxRoleCode">
              <template v-slot:default="{ active, }">
                ...
                <v-list-item-content>
                  ...
                  <div style="color: black; font-weight: bold" v-if="isEDXDistrictAdminSelected && newrole.edxRoleCode === edxDistrictAdminRole">EDX District Admin users will be set up with all district Roles</div>
                </v-list-item-content>
              </template>
            </v-list-item>
          </v-list-item-group>
          ...
          <v-card-text style="background-color: #e7ebf0;" v-if="deleteState">
            <v-row no-gutters>
              <v-col class="d-flex justify-center">
                <span style="font-size: medium; font-weight: bold; color: black" >Are you sure you want to remove this users access for the district?</span>
              </v-col>
            </v-row>
            <v-row no-gutters>
...
export default {
  ...
  props: {
    ...
    type: {
      validator(value) {
        return ['districtId'].includes(value);
      },
      type: String,
      required: true
    }
  ...
  data() {
    return {
      ...
      deleteState: false,
      relinkState: false,
      edxDistrictAdminRole: 'EDX_DISTRICT_ADMIN',
      selectedRoles: []
    };
  },
  methods: {
    roleDisabled(role) {
      if (role.edxRoleCode === this.edxDistrictAdminRole) {
        return false;
      }
      if (this.isEDXDistrictAdminSelected) {
        return true;
      }
    ...
    selectedRolesChanged() {
      ...
      this.selectedRoles = [this.edxDistrictAdminRole];
    },
    getButtonWidth() {
    ...
    getRoleLabel(curRole){
      if(this.districtRoles.length > 0) {
        return this.districtRoles.find((role) => role.edxRoleCode === curRole.edxRoleCode).label;
      }
      return '';
    },
    clickEditButton() {
    ...
    clickActionRelinkButton(userToRelink) {
      let userDistrict = userToRelink.edxUserDistricts.find(district => district.districtId === this.districtId);
    ...
    clickRemoveButton(userToRemove) {
      let userDistrict = userToRemove.edxUserDistricts.find(district => district.districtId === this.districtId);
      const payload = {params:
            {
    ...
    setUserRolesAsSelected() {
      ...
      this.districtRoles.forEach((role) => {
  ...
  computed: {
    ...mapState('edx', ['districtRoles']),
    isEDXDistrictAdminSelected() {
      return this.selectedRoles.includes(this.edxDistrictAdminRole);
    },
