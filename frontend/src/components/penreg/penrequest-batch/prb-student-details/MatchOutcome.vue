<template>
  ...
  <v-card v-else id="searchResults" elevation="0" tile width="100%">
    <v-row no-gutters justify="space-between" class="sticky">
      <v-col>
        <v-card-title>
          <span id="numberMatches" class="px-4"><strong>{{ studentPossibleMatches.length || 0 }} Matches</strong><v-btn @click="matchesExpanded=!matchesExpanded" icon><v-icon nudge-bottom="4" color="#003366">{{!matchesExpanded?'fa-angle-up':'fa-angle-down'}}</v-icon></v-btn></span>
        </v-card-title>
      </v-col>
      <v-col align-self="center">
        <v-row justify="end" class="mx-3">
          <TertiaryButton disabled id="compareButton" class="ma-0" text="Compare" icon="mdi-content-copy" @click.native="compare"></TertiaryButton>
          <TertiaryButton id="refreshButton" class="ma-0" iconStyle="mdi-flip-h" text="Refresh" icon="mdi-cached" @click.native="refresh"></TertiaryButton>
        </v-row>
      </v-col>
      <v-progress-linear
              id="loading"
