  ...
  >
    ...
    >
      ...
      <v-data-table
        ...
        @update:items-per-page="getSldData(students.studentID)"
      >
        <template #headers />
        <template #item="item">
          <tr>
...
export default {
  ...
  mounted() {
    this.studentRecords.forEach(student => {
      this.getSldData(student.studentID);
  ...
  methods: {
    ...
    addPEN() {
      ...
      this.getSldData(this.studentDataMap.get(this.penToAdd)?.studentID);
    ...
    getSldData(assignedStudentID) {
      ApiService.apiAxios
        ...
        .then(response => {
          ...
          this.storedSldData[assignedStudentID] = response.data?.content;
          this.updateSldRowDisplay(assignedStudentID, true);
          this.updateSldTableRows(assignedStudentID, 10);
        })
        .catch(error => {
          console.log(error);
          this.setFailureAlert(this.studentDNEErrorMessage);
