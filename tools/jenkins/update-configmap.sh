...
echo
echo Creating SCHOOL_INDEPENDENT_OFFSHORE_ADMIN role
curl -sX POST "https://$SOAM_KC/auth/admin/realms/$SOAM_KC_REALM_ID/roles" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TKN" \
  -d "{\"name\" : \"SCHOOL_INDEPENDENT_OFFSHORE_ADMIN\",\"description\" : \"Allows access to edit offshore and independent schools\",\"composite\" : false,\"clientRole\" : false,\"containerId\" : \"$SOAM_KC_REALM_ID\"}"
